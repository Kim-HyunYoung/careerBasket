...
<body>
	...
	<c:if test="${empty applyList}">
		<div
			...
			<div style="height: 200px; top: 80px; position: relative;">아직
				지원한 회사가 없습니다.</div>
		</div>
	...
	<c:if test="${not empty applyList}">
		...
		<div
			style="position: absolute; width: 1000px; left: 50%; margin-left: -525px; margin-top: 75px;">
			<!-- 지원 현황 테이블 -->
			<table class="table table-sm" style="margin: 40px; table-layout: fixed;">
					<colgroup>
						<col width="10%"/>
						<col width="12%"/>
						<col width="15%"/>
				...
				<tbody style="font-size: smaller;">
					<c:forEach var="apply" items="${applyList}">
						<tr>
							...
							<td style=" overflow: hidden; text-overflow: ellipsis;"><nobr>${apply.companyName}<nobr></td>
							<!-- 
							<td>${apply.hireTitle}</td>
							 -->

							<td style=" overflow: hidden; text-overflow: ellipsis;"><nobr>
							<a href='http://localhost:8080/hire/detail?id=${apply.id}&userId=${userId}'>${apply.hireTitle}<a><nobr></td>
							<td style=" overflow: hidden; text-overflow: ellipsis;"><nobr><a href='http://localhost:8080/resume/details?userId=${userId}&resumeId=${apply.resumeId}'>${apply.title}<a></a><nobr></td>
